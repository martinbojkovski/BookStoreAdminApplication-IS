...
namespace EShopAdminApplication.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            string URL = "https://localhost:44335/api/Admin/GetAllOrders";
        ...
        public IActionResult Details(string id)
        {
            HttpClient client = new HttpClient();
            //added in next aud
            string URL = "https://localhost:44335/api/Admin/GetDetails";
            var model = new
            {
                Id = id
            };
        ...
        public FileContentResult CreateInvoice(string id)
        {
            ...
            {
                sb.AppendLine("Book " + item.orderedBook.title + " has quantity " + item.quantity + " with price " + item.orderedBook.price);
                double price = (double) item.orderedBook.price;
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                for (int i = 0; i < data.Count(); i++)
                {
                    ...
                    for (int j = 0; j < item.bookInOrders.Count(); j++)
                    {
                        ...
                        worksheet.Cell(i + 2, 4 + j).Value = item.bookInOrders.ElementAt(j).orderedBook.title;
                        double price = (double)item.bookInOrders.ElementAt(j).orderedBook.price;
                        total += (item.bookInOrders.ElementAt(j).quantity * price);
                    }
